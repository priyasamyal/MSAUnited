...
export class AppointmentPage {
  ...
  getUpcomingAppointmentsListFunc() {
    if (!this.singleton.networkCheck()) {
      ...
      this.http.get(this.singleton.upcomingAppoinmentsListApi).subscribe(
        data => {
  ...
  getPastAppointmentsListFunc() {
    if (!this.singleton.networkCheck()) {
      ...
      this.singleton.showLoader();
      this.http.get(this.singleton.pastAppoinmentsListApi).subscribe(
        data => {
          this.data.response = data['_body'];
          var callback = JSON.parse(this.data.response);
          console.log(JSON.stringify(callback));
  ...
  joinChatFunc(sessionId) {
    if (!this.singleton.networkCheck()) {
      ...
      this.singleton.showLoader();
      this.http.post(this.singleton.sessionDetailApi, myData).subscribe(
        data => {
          this.data.response = data['_body'];
          var callback = JSON.parse(this.data.response);
          console.log(JSON.stringify(callback));
          ...
          if (callback.meta.success) {
            if (callback.data.session_detail.api_key == '') {
              ...
              if (years < 1 && months < 1 && days < 1 && hours < 1) {
                ...
                this.singleton.doAlert(
                  '',
                  'Chat is scheduled on ' +
                    callback.data.session_detail.date_time,
                  'Ok'
                );
              }
            } else {
  ...
  startTimer(secondsOnly, dateTime) {
    ...
    setTimeout(function() {
      document.getElementById('startAt').textContent =
        'Your session will start at ' + dateTime;
    }, 1000);
