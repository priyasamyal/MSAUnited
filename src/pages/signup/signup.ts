...
export class SignupPage {
  ...
  constructor(
    ...
    this.singleton.alertOpen = this.select;
    console.log(this.select, 'k');
    this.firstname = '';
    this.lastname = '';
    ...
    this.signup = formBuilder.group({
      ...
      acceptConditions: [
        false,
        Validators.compose([CheckboxValidator.isChecked, Validators.required]),
      ],
  ...
  onChange() {
    this.singleton.alertOpen = this.select;
    console.log(this.singleton.alertOpen, 'oncjane');
  }
  signupFunc() {
    let firstname = this.singleton.noSpaceWithCharacterOnlyPattern.test(
      this.firstname
    );
    let lastname = this.singleton.noSpaceWithCharacterOnlyPattern.test(
      this.lastname
    );
    let contactnumber = this.singleton.mobileNumberOnlyPattern.test(
      this.contactnumber
    );
    let email = this.singleton.emailPattern.test(this.email);
    let username = this.singleton.noSpaceWithAlphanumericUnderscoreOnlyPattern.test(
      this.username
    );

    if (!this.singleton.networkCheck()) {
      this.singleton.doAlert(
      ...
      this.singleton.commonToast(
        'Username can be alphanumeric including underscore.'
      );
      ...
      this.http.post(this.singleton.registerApi, myData).subscribe(
        data => {
