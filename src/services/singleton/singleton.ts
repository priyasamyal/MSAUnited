...
export class SingletonService {
  ...
  public upcomingAppoinmentsListApi: string = this.baseUrl +
  'userUpcomingAppoinmentsList/';
  public pastAppoinmentsListApi: string = this.baseUrl +
  'userPastAppoinmentsList/';
  ...
  equaltoFunc(field_name): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} => {
      ...
      if (!isValid) return {equalToKey: {isValid}};
      else return null;
    };
  }
  ...
  validateEmailFunc(): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} => {
      ...
      if (!emailTest) return {validateEmailKey: {emailTest}};
      else return null;
    };
  }
  ...
  noSpaceWithAlphanumericUnderscoreOnlyFunc(): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} => {
      ...
      let noSpaceWithAlphanumericUnderscoreOnlyTest = this.noSpaceWithAlphanumericUnderscoreOnlyPattern.test(
        input
      );
      ...
      if (!noSpaceWithAlphanumericUnderscoreOnlyTest)
        return {
          noSpaceAlphanumericUnerscoreOnlyKey: {
            noSpaceWithAlphanumericUnderscoreOnlyTest,
          },
        };
      else return null;
    };
  ...
  numberOnlyFunc(): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} => {
      ...
      if (!numberOnlyTest) return {numberOnlyKey: {numberOnlyTest}};
      else return null;
    };
  }
  ...
  characterOnlyWithSpaceFunc(): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} => {
      ...
      let characterOnlyWithSpaceTest = characterOnlyWithSpacePattern.test(
        input
      );
  ...
  getCommonDetail() {
    if (!this.networkCheck()) {
      ...
      this.http.get(this.commonDetailsApi).subscribe(
        data => {
          ...
          if (callback.meta.success) {
            ...
            for (let data of this.timezoneArr) {
              if (
                data.timezone_format &&
                data.timezone_format.substring(0, 1) != '-'
              ) {
                data.timezone_format = '+' + data.timezone_format;
              }
              if (data.timezone_format) {
                data['combined'] =
                  '(UTC' + data.timezone_format + ') ' + data.timezone_name;
              }
            }
            ...
            if (callback.data.errors.type == 'validation') {
              ...
              for (var propName in callback.data.errors.array) {
                if (callback.data.errors.array.hasOwnProperty(propName)) {
                  validationErrors +=
                    callback.data.errors.array[propName] + '<br>';
                }
              }
  ...
  getAppVersion() {
    let alert = this.alertCtrl.create({
      title: '',
      message:
        'A new version is available on App Store.\nPlease update your app.',
      buttons: [
        {
          text: 'UPDATE',
          role: 'update',
