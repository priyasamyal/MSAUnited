...
export class MyApp {
  ...
  constructor(
    ...
    if (window.localStorage.getItem('logged') == 'true') {
      ...
      setTimeout(function() {
        var userProfilePicture = document.getElementById(
          'userProfilePicture'
        ) as HTMLImageElement;
        userProfilePicture.src = window.localStorage.getItem(
          'userProfilePicture'
        );
      }, 500);
    } else {
  ...
  initializeApp() {
    ...
    this.platform.registerBackButtonAction(() => {
      ...
      console.log(view, 'view', activePortal);
      if (view == 1) {
        if (activePortal) {
        ...
        if (activeView != null) {
          ...
          if (activeView.component.name == 'SignupPage') {
            if (activePortal) {
              activePortal.dismiss();
            } else {
              nav.pop();
            }
          } else if (activeView.component.name == 'ProfilePage') {
            if (activePortal) {
              activePortal.dismiss();
            } else {
              nav.pop();
      ...
      fileEntry => {
        ...
        fileEntry.file(file => {
          ...
          reader.onloadend = e => {
            ...
            //make HTTP call
            this.http
              .post(this.singleton.saveProfileImageApi, doc_data)
              .subscribe(
                data => {
                  ...
                  if (callback.meta.success) {
                    ...
                    if (callback.data.errors.type == 'validation') {
                      ...
                      for (var propName in callback.data.errors.array) {
                        if (
                          callback.data.errors.array.hasOwnProperty(propName)
                        ) {
                          validationErrors +=
